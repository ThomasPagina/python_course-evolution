...
Main idea:
1. Generate a random binary starting tree from all living nodes.
2. In a loop, perform the following steps as long as the tree changes:
    a) Find an invalid edge (leaf-first).
        • Try a parent-child swap that results in no more invalidity violations.
        • If swap is not possible or insufficient, perform a "reattach" of the faulty leaf.
    b) If there are no more invalidities, check for overflow (nodes with >2 children):
        • Choose two children with minimal Levenshtein distance.
        • **Before** inserting a hypothetical node, check if one can be derived directly from the other (is_valid_mutation).
          In this case, simply attach one as a child under the other. Otherwise, insert a hypothetical node.
    c) If neither invalidity nor overflow exists, try to remove a redundant hypothetical node (bypass).
3. As soon as none of the three operations (a–c) causes a change, the tree is stable and is output as the final result.
...
class Node:
     ...
     Can be an original (living) node from the JSON file or a hypothetical node added during restructuring.
     """
...
class Tree:
     ...
     def is_valid_structure(self) -> bool:
          ...
          (Only structure, not mutation constraints.)
          """
          visited: Set[int] = set()
          root = self.get_root()
          if root is None:
...
def reattach_leaf(tree: Tree, parent_id: int, child_id: int, next_hypo_id: int) -> Tuple[Tree, int]:
     ...
     2. If parent_id is directly under root (gp=None), creates a new hypothetical root that unites parent_id and child_id.
...
def add_hypothetical_for_overflow(tree: Tree, next_hypo_id: int) -> Tuple[Tree, int]:
     ...
     or attaches one node directly under the other if that would be valid.
     """
     new_tree = tree.copy()
     ...
     # 1) Check if c2 can be a direct child of c1
     if is_valid_mutation(seq_c1, seq_c2):
          # Remove c2 from children of overflow_node and attach under c1
          new_tree.children_map[overflow_node].remove(c2)
          new_tree.parent_map[c2] = c1
          new_tree.children_map[c1].append(c2)
          return new_tree, next_hypo_id

     # 2) Check if c1 can be a direct child of c2
     ...
     # 3) Otherwise: create hypothetical node h
...
def single_step_mutation(tree: Tree, next_hypo_id: int) -> Tuple[Tree, int, bool]:
     ...
     1. Find an invalid edge (leaf-first).
         a) Try a direct parent-child swap; accept if after swap there is no more invalidity.
         b) Otherwise: reattach (including new root if necessary).
         → If a change occurred in step 1, return (new_tree, next_hypo_id, True).
     2. If there are no invalid edges, check for overflow:
         → If overflow exists, insert hypothetical node or attach directly under the other node,
            and return (new_tree, next_hypo_id, True).
