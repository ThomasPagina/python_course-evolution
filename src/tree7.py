...
Die neue Fitness-Funktion berechnet für jeden benachbarten Buchstabenpaar die alphabetische
Differenz (z. B. 'a'→'b' = +1, 'b'→'a' = -1) und summiert über den gesamten String.
"""
...
class TreeNode:
    ...
    def fitness(self) -> float:
        ...
        benachbarten Buchstaben im String. Für jedes Paar (s[i], s[i+1]) wird
        ord(s[i+1]) - ord(s[i]) berechnet und aufsummiert. Beispiel:
        - 'abc' → (ord('b')-ord('a')) + (ord('c')-ord('b')) = 1 + 1 = 2
        - 'acb' → (ord('c')-ord('a')) + (ord('b')-ord('c')) = 2 + (-1) = 1
        Je höher die Summe, desto fitter der Knoten.
        """
        total = 0
        seq = self.sequence.lower()
        for i in range(len(seq) - 1):
            total += (ord(seq[i + 1]) - ord(seq[i]))
        return float(total)
...
class Simulator:
    ...
    - Fitness-basierte Eliminierung basierend auf alphabetischer Distanz-Summe
    ...
    def _apply_elimination_exact(self) -> None:
        ...
        basierend auf der neuen Fitness-Funktion (alphabetische Differenz-Summe).
        """
        alive_ids = self.tree.alive_nodes()
        ...
        # Sortiere lebende Knoten nach Fitness aufsteigend (geringste Differenz-Summe zuerst)
