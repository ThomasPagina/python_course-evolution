...
# Reuse data structure definitions from the original problem
class Activity:
...
toolbox.register("attr_bool", lambda: random.choice([True, False]))
toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_bool, n=len(activities))
toolbox.register("population", tools.initRepeat, list, toolbox.individual)

def evaluate(individual: List[bool]) -> tuple:
...
def print_plan(individual: List[bool]):
    print("\nTim's Optimized Schedule (DEAP Version):\n")
    for active, activity in zip(individual, activities):
        status = "✔" if active else "✘"
        print(f"{status} {activity.name} on day {activity.day} at {activity.start}:00")
    print("\nMaximum Sun Hours:", evaluate(individual)[0])

if __name__ == "__main__":
    best_individual, max_score = run_deap_evolution()
