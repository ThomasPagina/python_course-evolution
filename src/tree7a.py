...
Die Fitness-Funktion berechnet für jedes benachbarte Buchstabenpaar:
- Bei aufsteigender Reihenfolge (z.B. 'a'→'b'): addiert die positive Differenz.
- Bei absteigender Reihenfolge (z.B. 'b'→'a'): Multipliziert die negative Differenz mit 1.5.
- Bei gleichen Buchstaben (z.B. 'a'→'a'): zieht 0.2 ab (Konstante Strafe).
"""
...
class TreeNode:
    ...
    def fitness(self) -> float:
        """
        Berechnet die Fitness anhand der alphabetischen Distanzen zwischen
        benachbarten Buchstaben im String:
        - Aufsteigende Paare (s[i] < s[i+1]): addiere (ord(s[i+1]) - ord(s[i])).
        - Absteigende Paare (s[i] > s[i+1]): addiere 1.5 * (ord(s[i+1]) - ord(s[i])).
        - Gleiche Buchstaben (s[i] == s[i+1]): subtrahiere 0.2 (Strafe).
        """
        total = 0.0
        seq = self.sequence.lower()
        for i in range(len(seq) - 1):
            diff = ord(seq[i + 1]) - ord(seq[i])
            if diff > 0:
                total += diff
            elif diff < 0:
                total += 1.5 * diff
...
class Simulator:
    ...
    - Fitness-basierte Eliminierung basierend auf alphabetischer Differenz-Summe
    ...
    def simulate(self) -> MutationTree:
        ...
        2. Für jede Generation:
           ...
           b) Eliminierung basierend auf Fitness (alphabetische Differenz-Summe) und Eliminationsrate.
        """
    ...
    def _get_unique_mutation(self, parent_sequence: str) -> str:
        ...
        while True:
            ...
            if mutated not in self._sequence_cache:
                return mutated
            # Sollte in extrem unwahrscheinlichen Fällen eintreten, selten endlos loopen.
            if attempts > 1000:
                continue
    ...
    def _apply_elimination_exact(self) -> None:
        ...
        basierend auf der neuen Fitness-Funktion (alphabetische Differenz-Summe mit Modifikatoren).
        """
        alive_ids = self.tree.alive_nodes()
        ...
        # Sortiere lebende Knoten nach Fitness aufsteigend (geringste Punktzahl zuerst)
        alive_nodes = [self.tree.get_node(nid) for nid in alive_ids]
        alive_nodes.sort(key=lambda node: node.fitness())

        # Wähle genau num_to_eliminate Knoten aus den am wenigsten fitten aus
